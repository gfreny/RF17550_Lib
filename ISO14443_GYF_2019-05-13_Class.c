...
#define Use_SystemNoDelay 0//0使用不堵塞的滴答计时，1使用通用的滴答计时

#if (Use_SystemNoDelay==1)//1使用通用的滴答计时
  extern volatile u32 	systicnum;
	#define TimeSatrt systicnum
	#define TimeLoop systicnum
#else //0使用不堵塞的滴答计时
	#define TimeSatrt System_ReadMSTimerStart()
	#define TimeLoop System_ReadMSTimerRuning()
#endif
...
/*	THM_WriteReg(DP1,0xc0);
	...
	THM_WriteReg(DN0,0x16);
	THM_WriteReg(CTR_RX,0xC0); 	          // Enable Digtal_TST and set 
	THM_WriteReg(EGT,0x00); 	          //  
	THM_WriteReg(CONTROL_TX,0x72);*/
}
...
{ 
	...
	Pcd_SetTimer(100);//时间另需精确计算
	
	Clear_FIFO();
	rvsta=Pcd_Comm(Transceive,c_RATS,2,ATS,&rece_bitlen);
	
	if (rvsta!=TRUE)
		return ERR_REV_ATS;
	
		ATSlen=rece_bitlen/8;
  //进行ATS分析
...
/*
	...
	{  		
		...
		if((TL>1)&&((T0&0x20) == 0x20))				 // if TB1 exists
		{
			...
			mypicc[mycid].FWT_ms=pow(2,mypicc[mycid].FWI)*3/10+1;	
		...
		//------2018-04-04--------------------
		if((TL>1)&&((T0&0x40) == 0x40))				 // if TC1 exists
		{
			if((ATS[4]&0x02) == 0x02)//CID
			{
				mypicc[mycid].withCID=1;
			}
			else
			{
				mypicc[mycid].withCID=0;
			}
...
s16	ISO14443_sendAPDU(u8 cid,u8 *tbuffer,u16 length,u8 *rbuffer)
{
  ...
  u32 timesave;//防止死机
	...
	sendtime +=1;
//	timesave=System_ReadMSTimerStart();
	timesave=TimeSatrt;//systicnum;
	while(1) 
	{	 
		//if(System_ReadMSTimerRuning()>200)//防止超时死机
		//if((systicnum-timesave)>500)//防止超时死机
		if((TimeLoop-timesave)>500)//防止超时死机
		...
		{ 			 
			...
			if((rPCB&0xC0) == 0x00){ 
				...
				//带不带CID的需要判断是否跟复位值一致
				if((rPCB&0x04)||((rPCB&0x08)!=(mypicc[cid].withCID<<3))||((rPCB&0x03)==(mypicc[cid].blocknumber&0x03))||(rLEN>FSD[FSDI])||PCDchaining)	
					return ERR_RECEIVE_SYNTAX;				
			...
			else if((rPCB&0xF0) == 0xF0)	 // TA 404  
			{
				...
				timesave=TimeSatrt;//systicnum;//重新计时
			}
			else//syntax error 			
				 return ERR_RECEIVE_SYNTAX;							
		}
...
{  
	...
	//M_exchange_crc(&blength, tempbuffer,&tempbuffer[50]);
}
...
s16	ISO14443_Attrib(u8 *ATQB,u8 mycid,u8 *rbuffer)
{
  ...
  mypicc[mycid].FWT_ms=pow(2,mypicc[mycid].FWI)*3/10+1;
	...
	ret=Pcd_Comm(Transceive,send_byte,9,rbuffer,&rLEN);
	
	if(ret==TRUE)
	{
		//if(rbuffer[0]!=send_byte[8])
		//if((rbuffer[0]&0x0F)!=0)//if CID!=0
		//	return ERR_REV_ATTRIB_RESPONSE;
		if (((ATQB_Save[11] & 0x01U) != 0U) && ((rbuffer[0] & 0x0FU) != mycid))
   	return -1;//2019-5-8

		//in EMV, CID is not used
		mypicc[mycid].MBLI=(rbuffer[0]>>4); //
...
static void sendBlock(u8 PCB,u8 cid,u8 *tbuffer,u16 length)  
{
		...
		//Pcd_SetTimer(500);//临时，需精确定时
		Pcd_SetTimer(mypicc[cid].FWT_ms*mypicc[cid].WTXM);
		Clear_FIFO();
...
//------------
void ATR2ATS(u16 Inlen,u8 *Indata,unsigned int  *Outlen,u8 *Outdata)
{
	int i;
	memcpy(Outdata,"\x3b\x8f\x80\x01",4);
	Outdata[1]=Indata[1] & 0x70;
	if((Indata[1] & 0x70) == 0x70)
		Outdata[1]=Inlen-5;
	else if((Indata[1] & 0x70) == 0x30 ||(Indata[1] & 0x70) == 0x50||(Indata[1] & 0x70) == 0x60)
		Outdata[1]=Inlen-4;
	else if((Indata[1] & 0x70) == 0x10 ||(Indata[1] & 0x70) == 0x20||(Indata[1] & 0x70) == 0x40)
		Outdata[1]=Inlen-3;
	else
		Outdata[1]=Inlen-2;
	
	memcpy(Outdata+4,Indata+Inlen-Outdata[1],Outdata[1]);
	
	*Outlen = 4 + Outdata[1]+1;
	Outdata[1] |=0x80;
	Outdata[*Outlen -1] =0;
	for(i=1;i<*Outlen -1;i++)
		Outdata[*Outlen -1] ^= Outdata[i];
}	
